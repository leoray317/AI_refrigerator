...
df = pd.read_csv('final1.csv',encoding = 'utf-8-sig')
...
for name in tqdm.tqdm(df_l.values):
    try:
        ...
        est1 = est.fit() 
        #print(est1.rsquared)
        train_test_regrassion.append([name[0],name[1],est1.rsquared])
    except:
        print('資料量過少')
        continue
...
train_test_df.to_csv('price_volumn_regrassion_just_volumn.csv',encoding = 'utf-8-sig',index = False)
